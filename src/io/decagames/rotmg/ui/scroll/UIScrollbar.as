	...
	public class UIScrollbar extends Sprite {
		...
		public function update() : void {
			...
			if(this._content.height <= this.contentHeight) {
				...
				loc1 = (1 - this.percent * SCROLL_SLIDER_SCALE_FACTOR) * this.contentHeight;
				if(loc1 < SCROLL_SLIDER_MINIMUM_HEIGHT) {
					loc1 = SCROLL_SLIDER_MINIMUM_HEIGHT;
				}
				this.sliderAsset.height = loc1;
			}
		}
		...
		public function updatePosition(param1:Number) : void {
			...
			var loc2:int = this.contentHeight - this._slider.height;
			if(this._slider.y > loc2) {
				this._slider.y = loc2;
			}
			if(loc2 > 0) {
				this._content.y = this.initalPosition + -Math.round((this._content.height - this.contentHeight) * this._slider.y / loc2);
			}
		}

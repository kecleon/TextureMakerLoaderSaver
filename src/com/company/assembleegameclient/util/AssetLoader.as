...
package com.company.assembleegameclient.util {
	...
	public class AssetLoader {
		...
		private function parse3DModels() : void {
			var loc1:* = null;
			var loc2:ByteArray = null;
			var loc3:String = null;
			for(loc1 in EmbeddedAssets.models_) {
				loc2 = EmbeddedAssets.models_[loc1];
				loc3 = loc2.readUTFBytes(loc2.length);
				Model3D.parse3DOBJ(loc1,loc2);
				Model3D.parseFromOBJ(loc1,loc3);
			}
		}
		
		private function parseParticleEffects() : void {
			var loc1:XML = XML(new EmbeddedAssets.particlesEmbed());
			ParticleLibrary.parseFromXML(loc1);
		}
		
		private function parseGroundFiles() : void {
			var loc1:* = undefined;
			for each(loc1 in EmbeddedData.groundFiles) {
				GroundLibrary.parseFromXML(XML(loc1));
			}
		}
		...
		private function parseObjectFiles() : void {
			var loc1:int = 0;
			while(loc1 < 25) {
				currentXmlIsTesting = this.checkIsTestingXML(EmbeddedData.objectFiles[loc1]);
				ObjectLibrary.parseFromXML(XML(EmbeddedData.objectFiles[loc1]));
				loc1++;
			}
			var loc2:int = 0;
			while(loc2 < EmbeddedData.objectFiles.length) {
				ObjectLibrary.parseDungeonXML(getQualifiedClassName(EmbeddedData.objectFiles[loc2]),XML(EmbeddedData.objectFiles[loc2]));
				loc2++;
			}
		...
		private function parseRegionFiles() : void {
			var loc1:* = undefined;
			for each(loc1 in EmbeddedData.regionFiles) {
				RegionLibrary.parseFromXML(XML(loc1));
			}
		}

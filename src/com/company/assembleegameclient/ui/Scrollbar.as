package com.company.assembleegameclient.ui {
	...
	public class Scrollbar extends Sprite {
		...
		public function Scrollbar(param1:int, param2:int, param3:Number = 1.0, param4:Sprite = null) {
			...
			graphicsData_ = new <IGraphicsData>[this.backgroundFill_, this.path_, GraphicsUtil.END_FILL];
			super();
			this.target_ = param4;
		...
		public function setIndicatorSize(param1:Number, param2:Number, param3:Boolean = true):void {
			var loc4:int = param2 == 0 ? int(this.indicatorRect_.height) : int(param1 / param2 * this.indicatorRect_.height);
			loc4 = Math.min(this.indicatorRect_.height, Math.max(this.width_, loc4));
			this.drawIndicator(this.width_, loc4, this.posIndicator_.graphics);
		...
		private function getSprite(param1:Function):Sprite {
			...
			loc2.addEventListener(MouseEvent.MOUSE_DOWN, param1);
			loc2.addEventListener(MouseEvent.ROLL_OVER, this.onRollOver);
			loc2.addEventListener(MouseEvent.ROLL_OUT, this.onRollOut);
			return loc2;
		}

		private function onRollOver(param1:MouseEvent):void {
			var loc2:Sprite = param1.target as Sprite;
			loc2.transform.colorTransform = new ColorTransform(1, 0.8627, 0.5216);
		}

		private function onRollOut(param1:MouseEvent):void {
			var loc2:Sprite = param1.target as Sprite;
			loc2.transform.colorTransform = new ColorTransform(1, 1, 1);
		}
		...
		private function onArrowFrame(param1:Event):void {
			...
			var loc3:Number = (loc2 - this.lastUpdateTime_) / 1000;
			var loc4:Number = (this.height_ - this.width_ * 3) * loc3 * this.change_;
			this.setPos((this.posIndicator_.y + loc4 - this.indicatorRect_.y) / (this.indicatorRect_.height - this.posIndicator_.height));
			this.lastUpdateTime_ = loc2;
		}
		...
		public function resize(param1:int, param2:int, param3:Number = 1.0):void {
			...
			this.height_ = param2;
			this.speed_ = param3;
			loc4 = this.width_ * 0.75;
			this.indicatorRect_ = new Rectangle(0, loc4 + 5, this.width_, this.height_ - loc4 * 2 - 10);
			var loc5:Graphics = this.background_.graphics;
			loc5.clear();
			loc5.beginFill(5526612, 1);
			loc5.drawRect(this.indicatorRect_.x, this.indicatorRect_.y, this.indicatorRect_.width, this.indicatorRect_.height);
			loc5.endFill();
			drawArrow(loc4, this.width_, this.upArrow_.graphics);
			this.upArrow_.rotation = -90;
			this.upArrow_.x = this.width_ / 2;
			this.upArrow_.y = loc4 / 2;
			drawArrow(loc4, this.width_, this.downArrow_.graphics);
			this.downArrow_.x = this.width_ / 2;
			this.downArrow_.y = this.height_ - loc4 / 2;

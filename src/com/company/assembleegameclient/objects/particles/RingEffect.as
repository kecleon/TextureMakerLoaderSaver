package com.company.assembleegameclient.objects.particles {
	...
	public class RingEffect extends ParticleEffect {
		...
		override public function runNormalRendering(param1:int, param2:int):Boolean {
			...
			var loc6:int = 0;
			while (loc6 < loc4) {
				loc7 = loc6 * 2 * Math.PI / loc4;
				loc8 = new Point(this.start_.x + this.novaRadius_ * Math.cos(loc7), this.start_.y + this.novaRadius_ * Math.sin(loc7));
				loc9 = new Point(this.start_.x + this.novaRadius_ * 0.9 * Math.cos(loc7), this.start_.y + this.novaRadius_ * 0.9 * Math.sin(loc7));
				loc10 = new SparkerParticle(loc3, this.color_, loc5, loc9, loc8);

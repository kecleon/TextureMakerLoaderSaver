...
package com.company.assembleegameclient.objects {
	...
	public class ProjectileProperties {
		...
		public function ProjectileProperties(param1:XML) {
			...
			for each(loc2 in param1.ConditionEffect) {
				if(this.effects_ == null) {
					this.effects_ = new Vector.<uint>();
				}
				this.effects_.push(ConditionEffect.getConditionEffectFromName(String(loc2)));
				if(loc2.attribute("target") == "1") {
					if(this.isPetEffect_ == null) {
						this.isPetEffect_ = new Dictionary();
					}
					this.isPetEffect_[ConditionEffect.getConditionEffectFromName(String(loc2))] = true;
				}
			}

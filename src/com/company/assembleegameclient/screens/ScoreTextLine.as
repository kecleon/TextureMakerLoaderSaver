package com.company.assembleegameclient.screens {
	...
	public class ScoreTextLine extends Sprite implements TooltipAble {
		...
		public function ScoreTextLine(param1:int, param2:uint, param3:uint, param4:String, param5:String, param6:int, param7:int, param8:String, param9:String, param10:DisplayObject) {
			...
			var loc11:LineBuilder = new LineBuilder().setParams(param4);
			if (param8 == "+") {
				loc11.setPrefix("Bonus: ");
			}
			this.nameText_.setStringBuilder(loc11);
		...
		public function onEnterFrame(param1:Event):void {
			var loc3:int = 0;
			var loc2:Number = Math.min(1, (getTimer() - this.startTime_) / 500);
			if (this.numberText_ != null) {
				loc3 = this.number_ * loc2;
				this.numberText_.setStringBuilder(new StaticStringBuilder(this.numberPrefix_ + loc3.toString() + " " + this.unit_));
		...
		private function makeDescription():StringBuilder {
			...
			loc1.setDelimiter("");
			loc1.pushParams(this.description);
			if (this.level > 1) {
				loc1.pushParams(TextKey.BLANK, {"data": " \n("});
				loc1.pushParams("FameBonus.LevelRequirement", {"level": this.level});
				loc1.pushParams(TextKey.BLANK, {"data": ")"});
			}
			return loc1;
		}

...
package com.company.assembleegameclient.screens {
	...
	public class GraveyardLine extends Sprite {
		...
		protected function onMouseDown(param1:MouseEvent) : void {
			var loc2:Array = this.link.split(":",2);
			switch(loc2[0]) {
				case "fame":
					this.viewCharacterFame.dispatch(int(loc2[1]));
					break;
				case "http":
				case "https":
				default:
		...
		private function getTimeDiff(param1:int) : String {
			var loc2:Number = new Date().getTime() / 1000;
			var loc3:int = loc2 - param1;
			if(loc3 <= 0) {
				return "now";
			}
			if(loc3 < 60) {
			...
			if(loc3 < 60 * 60) {
				return int(loc3 / 60) + " mins";
			}
			if(loc3 < 60 * 60 * 24) {
				return int(loc3 / (60 * 60)) + " hours";
			}
			return int(loc3 / (60 * 60 * 24)) + " days";
		}
	}
}

...
package kabam.lib.ui.impl {
	...
	public class LayoutList extends Sprite implements List {
		...
		public function getSizeOfItems() : Size {
			var loc1:Rectangle = this.container.getRect(this.container);
			return new Size(loc1.width,loc1.height);
		}
		
		private function applySizeToMask() : void {
			var loc1:Graphics = this.containerMask.graphics;
			loc1.clear();
			loc1.beginFill(10027263);
			loc1.drawRect(0,0,this.size.width,this.size.height);
			loc1.endFill();
		...
		private function clearList() : void {
			var loc1:int = this.list.length;
			while(loc1--) {
				this.container.removeChild(this.list[loc1]);
			}
			this.list.length = 0;
		}
		...
		private function addItemsToListAndContainer(param1:Vector.<DisplayObject>) : void {
			...
			for each(loc2 in param1) {
				this.addToListAndContainer(loc2);
			}
		}

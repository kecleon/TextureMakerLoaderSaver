package kabam.rotmg.ui.view {
	...
	public class HUDMediator extends Mediator {
		...
		private function onStatsMouseUp(param1:MouseEvent):void {
			var loc2:Sprite = StatsView(param1.target);
			this.stopDraggingStatsAsset(loc2);
			if (loc2.hitTestObject(this.view.tabStrip)) {
				this.dockStats(loc2);
			}
		}
		...
		private function onStatsMouseDown(param1:MouseEvent):void {
			var loc2:Sprite = Sprite(param1.target);
			this.stats = loc2;
			loc2.removeEventListener(MouseEvent.MOUSE_DOWN, this.onStatsMouseDown);
			loc2.addEventListener(MouseEvent.MOUSE_UP, this.onStatsMouseUp);
			loc2.startDrag();
		}

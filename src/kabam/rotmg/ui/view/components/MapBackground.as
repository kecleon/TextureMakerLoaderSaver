package kabam.rotmg.ui.view.components {
	...
	public class MapBackground extends Sprite {
		...
		private function makeMap():Map {
			var loc1:ByteArray = new EMBEDDED_BACKGROUNDMAP();
			var loc2:String = loc1.readUTFBytes(loc1.length);
			mapSize = MapDecoder.getSize(loc2);
			xVal = BORDER;
			yVal = BORDER + int((mapSize.y_ - 2 * BORDER) * Math.random());
			camera = new Camera();
			var loc3:Map = new Map(null);
			loc3.setProps(mapSize.x_ + 2 * BORDER, mapSize.y_, "Background Map", Background.NO_BACKGROUND, false, false);
			loc3.initialize();
			MapDecoder.writeMap(loc2, loc3, 0, 0);
			MapDecoder.writeMap(loc2, loc3, mapSize.x_, 0);
			return loc3;
		}
	}
}

...
package kabam.rotmg.ui.view.components {
	...
	public class PotionSlotView extends Sprite {
		...
		public function PotionSlotView(param1:Array, param2:int) {
			...
			loc3 = AssetLibrary.getImageFromSet("lofiObj3",225);
			loc3 = TextureRedrawer.redraw(loc3,30,false,0);
		...
		public function setData(param1:int, param2:int, param3:Boolean, param4:int = -1) : void {
			...
			if(param4 != -1) {
				...
				loc7 = ObjectLibrary.getRedrawnTextureFromType(param4,55,false);
				this.potionIcon = new Bitmap(loc7);
				this.potionIcon.y = -11;
				addChild(this.potionIcon);
				loc7 = ObjectLibrary.getRedrawnTextureFromType(param4,80,true);
				loc8 = new Bitmap(loc7);
				loc8.x = loc8.x - 30;
				loc8.y = loc8.y - 30;
				this.potionIconDraggableSprite.addChild(loc8);
			}
			var loc5:* = param1 > 0;
			if(loc5) {
		...
		private function onMouseMoveCheckDrag(param1:MouseEvent) : void {
			var loc2:Number = param1.stageX - this.dragStart.x;
			var loc3:Number = param1.stageY - this.dragStart.y;
			var loc4:Number = Math.sqrt(loc2 * loc2 + loc3 * loc3);
			if(loc4 > DRAG_DIST) {

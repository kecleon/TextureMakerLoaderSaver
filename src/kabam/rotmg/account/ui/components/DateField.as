package kabam.rotmg.account.ui.components {
	...
	public class DateField extends Sprite {
		...
		private function drawSimpleTextBackground(param1:BaseSimpleText, param2:int, param3:int, param4:Boolean):void {
			var loc5:uint = !!param4 ? uint(ERROR_BORDER_COLOR) : uint(NORMAL_BORDER_COLOR);
			graphics.lineStyle(2, loc5, 1, false, LineScaleMode.NORMAL, CapsStyle.ROUND, JointStyle.ROUND);
		...
		private function createFormatHint(param1:BaseSimpleText, param2:String):TextFieldDisplayConcrete {
			var loc3:TextFieldDisplayConcrete = new TextFieldDisplayConcrete().setSize(16).setColor(FORMAT_HINT_COLOR);
			loc3.setTextWidth(param1.width + 4).setTextHeight(param1.height);
			loc3.x = param1.x - 6;
			loc3.y = param1.y + 3;
			loc3.setStringBuilder(new LineBuilder().setParams(param2));
			loc3.setAutoSize(TextFieldAutoSize.CENTER);
			return loc3;
		}

		private function onMonthInput(param1:TextEvent):void {
			var loc2:String = this.months.text + param1.text;
			var loc3:int = int(loc2);
			if (loc2 != "0" && !this.validator.isValidMonth(loc3)) {
				param1.preventDefault();
			}
		}

		private function onMonthFocusOut(param1:FocusEvent):void {
			var loc2:int = int(this.months.text);
			if (loc2 < 10 && this.days.text != "") {
				this.months.text = "0" + loc2.toString();
			}
		}
		...
		private function onDayInput(param1:TextEvent):void {
			var loc2:String = this.days.text + param1.text;
			var loc3:int = int(loc2);
			if (loc2 != "0" && !this.validator.isValidDay(loc3)) {
				param1.preventDefault();
			}
		}

		private function onDayFocusOut(param1:FocusEvent):void {
			var loc2:int = int(this.days.text);
			if (loc2 < 10 && this.days.text != "") {
				this.days.text = "0" + loc2.toString();
			}
		}
		...
		private function onYearInput(param1:TextEvent):void {
			var loc2:String = this.years.text + param1.text;
			var loc3:int = this.getEarliestYear(loc2);
			if (loc3 > this.thisYear) {
				param1.preventDefault();
			}
		}
		...
		public function isValidDate():Boolean {
			var loc1:int = int(this.months.text);
			var loc2:int = int(this.days.text);
			var loc3:int = int(this.years.text);
			return this.validator.isValidDate(loc1, loc2, loc3, 100);
		}

		public function getDate():String {
			var loc1:String = this.getFixedLengthString(this.months.text, 2);
			var loc2:String = this.getFixedLengthString(this.days.text, 2);
			var loc3:String = this.getFixedLengthString(this.years.text, 4);

...
package kabam.rotmg.game.view.components {
	...
	public class StatsView extends Sprite {
		...
		private function createStats() : void {
			var loc3:StatView = null;
			var loc1:int = 0;
			var loc2:int = 0;
			while(loc2 < statsModel.length) {
				loc3 = this.createStat(loc2,loc1);
				this.stats_.push(loc3);
				this.containerSprite.addChild(loc3);
				loc1 = loc1 + loc2 % 2;
				loc2++;
			}
		}
		...
		private function createStat(param1:int, param2:int) : StatView {
			var loc4:StatView = null;
			var loc3:StatModel = statsModel[param1];
			loc4 = new StatView(loc3.name,loc3.abbreviation,loc3.description,loc3.redOnZero);
			loc4.x = param1 % 2 * this.WIDTH / 2;
			loc4.y = param2 * (this.HEIGHT / 3);
			return loc4;
		}

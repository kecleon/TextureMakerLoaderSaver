...
package kabam.rotmg.game.commands {
	...
	public class UseBuyPotionCommand {
		...
		private function canPurchasePotion() : Boolean {
			var loc1:* = this.player.credits_ >= this.potion.currentCost(this.player.getPotionCount(this.potionId));
			var loc2:Boolean = Parameters.data_.contextualPotionBuy;
			return loc1 && loc2;
		}
		...
		private function sendServerRequest() : void {
			var loc1:int = PotionInventoryModel.getPotionSlot(this.vo.objectId);
			var loc2:int = Parameters.data_.contextualPotionBuy;
			this.gsc.useItem(getTimer(),this.player.objectId_,loc1,this.potionId,this.player.x_,this.player.y_,loc2);

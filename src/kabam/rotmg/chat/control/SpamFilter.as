...
package kabam.rotmg.chat.control {
	public class SpamFilter {
		...
		private function createFilterTable() : Array {
			...
			var loc1:Array = [];
			var loc2:Array = ["Aa","Bb","Cc","Dd","Ee","Ff","Gg","Hh","Ii","Jj","Kk","Ll","Mm","Nn","Oo","Pp","Qq","Rr","Ss","Tt","Uu","Vv","Ww","Xx","Yy","Zz","00","11","22","33","44","55","66","77","88","99"];
			for each(loc3 in loc2) {
				loc1[loc3.charCodeAt(0)] = loc3.charAt(1);
				loc1[loc3.charCodeAt(1)] = loc3.charAt(1);
			}
			return loc1;
		}
		...
		public function setPatterns(param1:Array) : void {
			...
			var loc2:String = this.patterns.join("|");
			this.regex = new RegExp(loc2,"g");
			this.evalAreValuesIdentical();
		}
		
		private function evalAreValuesIdentical() : void {
			var loc1:uint = 0;
			while(loc1 < this.testPatterns.length) {
				loc1++;
			}
		}
		...
		public function isSpam(param1:String) : Boolean {
			...
			var loc2:String = this.applyFilterOn(param1);
			var loc3:Boolean = this.regex.test(loc2);
			return loc3;
		}
		...
		private function applyFilterOn(param1:String) : String {
			...
			while(loc3 < param1.length) {
				loc4 = this.filterTable[param1.charCodeAt(loc3)];
				if(loc4 !== null) {
					loc2.push(loc4);
				}
				loc3++;
		...
		private function returnAppropriatePatterns() : Array {
			...
			var loc4:uint = 0;
			var loc1:uint = 0;
			while(loc1 < this.newPatterns.length) {
				loc3 = "";
				loc4 = 0;
				while(loc4 < this.newPatterns[loc1].length) {
					if(this.newPatterns[loc1].charAt(loc4) != "\'" && this.newPatterns[loc1].charAt(loc4) != "\"") {
						loc3 = loc3.concat(this.newPatterns[loc1].charAt(loc4));
					}
					loc4++;
				}
				this.newPatterns[loc1] = loc3;
				loc1++;
			}
			var loc2:Array = this.newPatterns;
			loc2.sort();

...
package kabam.rotmg.chat.view {
	...
	public class ChatInputMediator extends Mediator {
		...
		private function makeTextfield() : TextField {
			var loc1:TextField = this.textAndMapProvider.getTextField();
			this.fontModel.apply(loc1,14,16777215,true);
			return loc1;
		}
		...
		private function processKeyUp(param1:KeyboardEvent) : void {
			...
			var loc2:uint = param1.keyCode;
			if(loc2 == this.chatShortcutModel.getCommandShortcut()) {
				this.view.activate("/",true);
			} else if(loc2 == this.chatShortcutModel.getChatShortcut()) {
				this.view.activate(null,true);
			} else if(loc2 == this.chatShortcutModel.getGuildShortcut()) {
				this.view.activate("/g ",true);
			} else if(loc2 == this.chatShortcutModel.getTellShortcut()) {
				this.handleTell();
			}
		}

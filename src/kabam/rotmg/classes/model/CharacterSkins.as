...
package kabam.rotmg.classes.model {
	public class CharacterSkins {
		...
		public function updateSkins(param1:int) : void {
			...
			for each(loc2 in this.skins) {
				this.updateSkinState(loc2);
			}
		}
		...
		public function getListedSkins() : Vector.<CharacterSkin> {
			var loc2:CharacterSkin = null;
			var loc1:Vector.<CharacterSkin> = new Vector.<CharacterSkin>();
			for each(loc2 in this.skins) {
				if(loc2.getState() != CharacterSkinState.UNLISTED) {
					loc1.push(loc2);
				}
			}
			return loc1;

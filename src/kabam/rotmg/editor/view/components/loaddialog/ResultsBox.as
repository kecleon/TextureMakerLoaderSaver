...
package kabam.rotmg.editor.view.components.loaddialog {
	...
	public class ResultsBox extends Sprite {
		...
		public function ResultsBox(param1:XML, param2:Boolean) {
			super();
			this.id_ = String(param1.@id);
			this.name_ = param1.PicName;
			this.pictureType_ = int(param1.DataType);
			this.tags_ = param1.Tags;
			this.mine_ = param1.hasOwnProperty("Mine");
			this.admin_ = param2;
			this.bitmap_ = new Bitmap();
			addChild(this.bitmap_);
			this.nameText_ = new BaseSimpleText(12,11776947,false,98,20);
		...
		public function makeBitmapData(param1:ByteArray) : void {
			this.bitmapData_ = PNGDecoder.decodeImage(param1);
		...
		private function showAnimation() : void {
			var loc1:int = this.bitmapData_.width / 7;
			var loc2:int = this.bitmapData_.height;
			this.animatedChar_ = new AnimatedChar(new MaskedImage(this.bitmapData_,null),loc1,loc2,AnimatedChar.RIGHT);
		...
		private function updateAnimation() : void {
			...
			var loc2:int = loc1 / 400 % AnimatedChar.NUM_ACTION;
			var loc3:Number = loc1 % 400 / 400;
			var loc4:MaskedImage = this.animatedChar_.imageFromDir(AnimatedChar.RIGHT,loc2,loc3);
			this.bitmap_.bitmapData = TextureRedrawer.redraw(loc4.image_,100,true,0,true);
		...
		private function showTextile() : void {
			var loc1:AnimatedChar = AnimatedChars.getAnimatedChar("players",0);
			...
			var loc3:BitmapData = TextureRedrawer.resize(loc2.image_,loc2.mask_,100,true,4294967295,0);
			loc3 = GlowRedrawer.outlineGlow(loc3,0,1.4,false);
			this.bitmap_.bitmapData = loc3;
			this.bitmap_.x = WIDTH / 2 - this.bitmap_.width / 2;
			this.bitmap_.y = HEIGHT / 2 - this.bitmap_.height / 2 - 10;
		}
		...
		private function addToolTip() : void {
			...
			var loc1:String = this.tags_.split(",").join(", ");
			var loc2:String = "Type: " + PictureType.TYPES[this.pictureType_].name_ + "\n" + "Size: " + this.bitmapData_.width + " x " + this.bitmapData_.height + "\n" + "Tags: " + loc1 + "\n" + "Id: " + this.id_;
			toolTip_ = new TextToolTip(3552822,10197915,this.name_,loc2,200);

...
package kabam.rotmg.legends.view {
	...
	public class LegendListItem extends Sprite {
		...
		private function makePlaceText() : void {
			...
			var loc1:String = this.legend.place == -1?"---":this.legend.place.toString() + ".";
			this.placeText.setBold(this.legend.place != -1);
			this.placeText.setStringBuilder(new StaticStringBuilder(loc1));
		...
		private function makeInventory() : void {
			...
			var loc1:IIterator = this.inventoryGrid.createInteractiveItemTileIterator();
			while(loc1.hasNext()) {
				loc2 = InteractiveItemTile(loc1.next());
				loc2.setInteractive(false);
			}
		...
		private function makeFameIcon() : void {
			...
			this.fameIcon = new Bitmap(TextureRedrawer.redraw(loc1,40,true,0));
			this.fameIcon.x = 630;
